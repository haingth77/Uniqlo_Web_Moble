...
test.describe('Home Page', () => {
  test('HP-001: Verify that home page is dependend on location of browser', async ({ appPage }) => {
    await expect(appPage).toHaveURL(AppUrl.Default);
  });

  test(`HP-002: Verify that 'Category' is displayed when hover on 'Women' - 'Men' - 'Kids' - 'Baby' tab`, async ({
    appPage,
  }) => {
